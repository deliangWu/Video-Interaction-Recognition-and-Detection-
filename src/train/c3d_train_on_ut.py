...
def main(argv):
    ...
    frmSize = (112,128,3)
    with tf.variable_scope('top') as scope:
        c3d = network.C3DNET(numOfClasses, frmSize,nof_conv1=48, nof_conv2= 128, nof_conv3=256, nof_conv4= 512, noo_fc6=4096, noo_fc7=4096)
    # ***********************************************************
    # define session
    # ***********************************************************
    config = tf.ConfigProto()
        ...
        with sess.as_default():
            ...
            if len(argv) < 2 or argv[1] == 'train' or argv[1] == 'Train':
                ...
                while True:
                    ...
                    epoch = ut_set.getEpoch()
                    learning_rate = 0.008 * 2**(-int(epoch/4))
                    c3d.train(train_x, train_y, sess, learning_rate=learning_rate)
