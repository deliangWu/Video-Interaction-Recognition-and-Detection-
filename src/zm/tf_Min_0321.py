...
rtime.Wdatetime('time_1.txt','start')
...
def define_model(train_samples,name):
    ...
    network=tl.layers.InputLayer(inputs=train_samples,name=name+'input_layer')
    network=tl.layers.Conv2dLayer(network,shape=[5,5,3,32],strides=[1,1,1,1],padding='SAME', \
                                  W_init=tf.contrib.layers.xavier_initializer(),name=name+'conv1')
    network=tl.layers.PoolLayer(network,ksize=[1,3,3,1],strides=[1,2,2,1],pool=tf.nn.max_pool,name=name+'max_pool')
    network.outputs=tf.nn.relu(network.outputs,name=name+'relu1')
    network.outputs=tf.nn.lrn(network.outputs,depth_radius=3,bias=1,alpha=5e-5,beta=0.75,name=name+'norm1')
    network.outputs=tf.nn.dropout(network.outputs,.8,name=name+'dropout1')
    network=tl.layers.Conv2dLayer(network,shape=[5,5,32,32],strides=[1,1,1,1],padding='SAME', \
                                  W_init=tf.contrib.layers.xavier_initializer(),name=name+'conv2')
    network=tl.layers.PoolLayer(network,ksize=[1,3,3,1],strides=[1,2,2,1],pool=tf.nn.avg_pool,name=name+'mean_pool1')
    network.outputs=tf.nn.relu(network.outputs,name=name+'relu2')
    network.outputs=tf.nn.lrn(network.outputs,depth_radius=3,bias=1,alpha=5e-5,beta=0.75,name=name+'norm2')
    network.outputs=tf.nn.dropout(network.outputs,.8,name=name+'dropout2')
    network=tl.layers.Conv2dLayer(network,shape=[5,5,32,64],strides=[1,1,1,1],padding='SAME', \
                                  W_init=tf.contrib.layers.xavier_initializer(),name=name+'conv3')
    network.outputs=tf.nn.relu(network.outputs,name=name+'relu3')
    network=tl.layers.PoolLayer(network,ksize=[1,3,3,1],strides=[1,2,2,1],pool=tf.nn.avg_pool,name=name+'mean_pool2')
    network = tl.layers.FlattenLayer(network, name=name+'flatten_layer')
    network=tl.layers.DenseLayer(network,n_units=500,W_init=tf.contrib.layers.xavier_initializer(), \
                                 name=name+'fp1')
    network.outputs=tf.nn.dropout(network.outputs,.7,name=name+'dropout3')
    network.outputs=tf.nn.relu(network.outputs,name=name+'relu4')
    network=tl.layers.DenseLayer(network,n_units=12,W_init=tf.contrib.layers.xavier_initializer(), \
                                 name=name+'fp2')
...
with tf.Session() as sess:
    ...
    with tf.device('/gpu:0'):
        ...
        np.savetxt("losslist_1.txt", losslist, newline='\n')
        rtime.Wdatetime('time_1.txt','end')
